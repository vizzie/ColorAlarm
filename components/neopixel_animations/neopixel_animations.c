...
static SemaphoreHandle_t s_anim_lock = NULL;
...
static void anim_lock(void) {
    if (s_anim_lock) {
        xSemaphoreTake(s_anim_lock, portMAX_DELAY);
    }
}

static void anim_unlock(void) {
    if (s_anim_lock) {
        xSemaphoreGive(s_anim_lock);
    }
}

static void ensure_anim_lock(void) {
    if (!s_anim_lock) {
        s_anim_lock = xSemaphoreCreateMutex();
    }
}
...
static void anim_task(void *arg) {
    ...
    while (1) {
        anim_lock();
        neopixel_anim_mode_t mode = s_mode;
        neopixel_t *strip = s_strip;
        anim_unlock();
        ...
        switch (mode) {
            ...
            case NEOPIXEL_ANIM_PULSE: {
                ...
                for (int i=0;i<strip->count;i++) neopixel_set_pixel(strip,i,s_r,s_g,s_b,0);
                neopixel_show(strip);
                vTaskDelay(pdMS_TO_TICKS(500));
                for (int i=0;i<strip->count;i++) neopixel_set_pixel(strip,i,0,0,0,0);
                neopixel_show(strip);
                vTaskDelay(pdMS_TO_TICKS(500));
                t += 1000;
                break;
            ...
            case NEOPIXEL_ANIM_RAINBOW: {
                ...
                for (int i=0;i<strip->count;i++) {
                    neopixel_set_pixel(strip,i,r,g,b,0);
                }
                neopixel_show(strip);
            ...
            case NEOPIXEL_ANIM_FADE_TO_SOLID: {
                ...
                for (int i=0;i<strip->count;i++) {
                    ...
                    if (s_fade_start) {
                        ...
                        if (strip->use_rgbw) sw = sp[3];
                    } else {
                        // no snapshot → assume start = current buffer values
                        uint8_t *sp = &strip->pixels[i*bpp];
                        sg = sp[0]; sr = sp[1]; sb = sp[2];
                        if (strip->use_rgbw) sw = sp[3];
                    }
                    ...
                    uint8_t nw = strip->use_rgbw
                                 ? (uint8_t)(sw + (int)((int)s_target_w - (int)sw) * u)
                                 : 0;
                    neopixel_set_pixel(strip, i, nr, ng, nb, nw);
                }
                anim_unlock();
                neopixel_show(strip);
            ...
            case NEOPIXEL_ANIM_RAINBOW_SMOOTH: {
                ...
                if (s_rainbow_gradient && strip->count > 1) {
                    for (int i = 0; i < strip->count; i++) {
                        float h = base_h + (360.0f * ((float)i / (float)strip->count));
                        if (h >= 360.0f) h -= 360.0f;
                        uint8_t rr, gg, bb;
                        hsv_to_rgb(h, s_rainbow_sat, s_rainbow_val, &rr, &gg, &bb);
                        neopixel_set_pixel(strip, i, rr, gg, bb, 0);
                    }
                } else {
                    uint8_t rr, gg, bb;
